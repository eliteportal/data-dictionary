...
async function getRequestedCSV(schema_url) {
  //format url
  let url = new URL(
    "https://schematic.api.sagebionetworks.org/v1/visualize/attributes"
  );
...
async function getRequestedJson(schema_url) {
  //format url
  let url = new URL(
    "https://schematic.api.sagebionetworks.org/v1/visualize/tangled_tree/layers"
  );
  url.searchParams.append("schema_url", schema_url);
  url.searchParams.append("figure_type", "component");
  //   console.log(url.toString());dependency
